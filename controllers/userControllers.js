...
exports.createUserAndSession = async (req, res, next) => {
  ...
  try {
    ...
    const userInfo = await UserInfo.create({role: 'user', user_id: user.id})
  ...
  res.redirect('/user/hello') // ответ 200 + автоматическое создание и отправка cookies в заголовке клиенту
  console.log(user)
  // res.render('infoMessage', user)
};
...
exports.checkUserAndCreateSession = async (req, res, next) => {
  ...
  try {
    ...
    user = await User.findOne({ where: { name: name }, raw: true });
